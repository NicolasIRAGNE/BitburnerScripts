...
export class Node extends lib.Host
{
    ...
    async run(task, factor = 1)
    {
        ...
        else
        {
            ...
            {
                await this.scp(dependency);
            }
            let args = [...task.args];
            if (task.policy !== undefined && task.policy.pass_threads === true)
                args.push(factor);
            if (task.policy !== undefined && task.policy.no_args === true)
                args = [];
            let success = await t.prof(this.exec, this, task.script, factor, args);
        }
...
export class WorkloadManager
{
    ...
    async update()
    {
        ...
        for (let node of this.nodes)
        {
            ...
            this.maxCost = Math.max(this.maxCost, node.maxRam);
        }
    }
