...
export class WorkloadManager
{
    ...
    async update_network()
    {
        ...
        for (let host of hosts)
        {
            ...
            if (!found)
            {
                this.ns.tprint(`Host type: ${host.type}`);
                let new_node = await createNode(this.ns, host.name);
                this.ns.tprint(`Node type before assignment: ${new_node.type}`);
                new_node = Object.assign(new_node, host);
                this.ns.tprint(`Node type after assignment: ${new_node.type}`);
        ...
        await this.update();
        // this.ns.tprint(`Updated network: ${this.nodes.length} nodes, ${lib.format_ram(this.ns, this.totalAvailableRam)} RAM available`);
    }
