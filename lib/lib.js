...
export const HostType =
{
    ...
    HOME: 1 << 2,
    /** The host is a non-purchased server. */
    NON_OWN: 1 << 3,
};
...
export class Host
{
    ...
    async update()
    {
        try
        {
            let s = this.ns.getServer(this.name);
            // copy all fields from the server object to this object
            for (let key in s)
            {
                this[key] = s[key];
            }
        ...
        {
            // this.ns.toast(`Error updating ${this.name}: server was probably deleted`, "error", 1000);
            return ;
        }
        if (this.purchasedByPlayer)
            this.type |= HostType.OWN;
        else
            this.type |= HostType.NON_OWN;
        if (this.name === "home")
    ...
    {
        this.executions++;
        await t.prof(this.scp, this, cmd);
        let res = await t.prof(this._exec, this, cmd, threads, args);
        await this.update();
        return res;
    }
...
export async function scrap_host(ns, hostname, dst, ignored_files = [])
{
    ...
    // if ends with .js, ignore it
    files = files.filter(file => file.endsWith(".js") === false);
    ...
    for (let file of files)
    {
        ...
        await ns.tprint(`\tFound ${file} on ${hostname}...`);
    }
}
